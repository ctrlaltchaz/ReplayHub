...
export class IncidentsController {
  ...
  async createIncident(@Req() req: any, @Body() dto: CreateIncidentDto): Promise<IncidentResponse> {
  ...
  async getIncidents(@Req() req: any, @Query() queryDto: QueryIncidentsDto) {
    const actualTenantId = req.tenant.id;
  ...
  async getIncident(@Req() req: any, @Param('id') id: string): Promise<IncidentResponse> {
    const actualTenantId = req.tenant.id;
  ...
  @ApiResponse({ status: 404, description: 'Incident not found' })
  async deleteIncident(@Req() req: any, @Param('id') id: string): Promise<{ message: string }> {
    const actualTenantId = req.tenant.id;
    await this.incidentsService.deleteIncident(
      actualTenantId,
      id,
      req.orgUser?.id,
      req.orgUser?.email ?? req.globalUser?.email ?? null

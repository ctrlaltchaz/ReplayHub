 ...
 */
export function ImpersonationBanner({ orgName, enabled = true }: ImpersonationBannerProps) {
  // Only fetch impersonation status for users allowed to impersonate (global admins).
  ...
  return (
    <div className="bg-yellow-500 text-yellow-950 px-4 py-2 text-sm flex items-center justify-between shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <span>
          You are impersonating <strong>{orgName || "this organization"}</strong> as a Global
          Administrator
        </span>
      </div>
      <Button

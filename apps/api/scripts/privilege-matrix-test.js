...
async function runPrivilegeMatrix() {
    ...
    const testRoutes = [
        ...
        { name: 'POST inventory/items', method: 'POST', url: '/org/testorg/inventory/items', data: { name: 'Test Item', quantity: 1 } },
        { name: 'POST inventory/items/:id/move', method: 'POST', url: '/org/testorg/inventory/items/test-id/move', data: { quantity: 1, location: 'New Location' } },
        { name: 'POST inventory/items/:id/book', method: 'POST', url: '/org/testorg/inventory/items/test-id/book', data: { quantity: 1, bookedBy: 'test' } },
        { name: 'POST assets/upload', method: 'POST', url: '/org/testorg/assets/upload', data: 'fake file data' },
        { name: 'PUT assets/:id', method: 'PUT', url: '/org/testorg/assets/test-id', data: { name: 'Updated Asset' } },
        { name: 'GET assets/:id/download', method: 'GET', url: '/org/testorg/assets/test-id/download' }
    ];

    // Expected matrix results (status codes)
    const expectedMatrix = {
        'GET inventory/items': { no_role: 403, viewer: 200, uploader: 403, manager: 200, ops_admin: 200 },
        'POST inventory/items': { no_role: 403, viewer: 403, uploader: 403, manager: 201, ops_admin: 201 },
        'POST inventory/items/:id/move': { no_role: 403, viewer: 403, uploader: 403, manager: 201, ops_admin: 201 },
        'POST inventory/items/:id/book': { no_role: 403, viewer: 403, uploader: 403, manager: 201, ops_admin: 201 },
        'POST assets/upload': { no_role: 403, viewer: 403, uploader: 201, manager: 201, ops_admin: 201 },
        'PUT assets/:id': { no_role: 403, viewer: 403, uploader: 403, manager: 200, ops_admin: 200 },
        'GET assets/:id/download': { no_role: 403, viewer: 403, uploader: 200, manager: 200, ops_admin: 200 }
    };
    ...
    testRoutes.forEach(route => {
        ...
        console.log(`${name} |   ${row.no_role || '???'}   |  ${row.viewer || '???'}   |   ${row.uploader || '???'}    |   ${row.manager || '???'}   |   ${row.ops_admin || '???'}   `);
    });

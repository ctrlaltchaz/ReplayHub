    ...
    try {
        ...
        const opsAdminPerms = allPermissions.filter(p =>
            ['users.create', 'users.read', 'users.update', 'users.delete',
                ...
                'invites.create', 'invites.read', 'invites.delete'].includes(p.key)
        );
        ...
        const producerPerms = allPermissions.filter(p =>
            ['users.read', 'roles.read'].includes(p.key)
        );

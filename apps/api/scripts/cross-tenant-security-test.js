...
async function testCrossTenantSecurity() {
  ...
  try {
    ...
    const orgALoginResponse = await axios.post(
    ...
    if (orgACookie) {
      ...
      console.log(
        `Cross-tenant access (A→B): ${crossTenantResponse.status} ${crossTenantResponse.statusText}`
      );
  ...
  console.log(
    `No-session access      | ${allUnauthorized ? '401' : 'MIXED'} | ${allUnauthorized ? '✅ Properly protected' : '❌ Security gap'}`
  );
  ...
  for (const test of tenantTests) {
    try {
      ...
      console.log(
        `${test.from} → ${test.to}: ${response.status} (${response.status === 404 || response.status === 403 ? '✅' : '❌'})`
      );
    } catch (error) {

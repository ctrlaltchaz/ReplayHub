...
class Phase4Verifier {
    ...
    async testB_LineupMembership() {
        ...
        try {
            ...
            try {
                const response = await this.client.post(`/api/org/testorg/lineups/${this.testData.lineupId}/slots`, {
                    slots: [{ playerId: this.testData.p3Id }],
    ...
    async testC_PublishImmutability() {
        ...
        try {
            // Publish lineup
            const publishResponse = await this.client.post(`/api/org/testorg/lineups/${this.testData.lineupId}/publish`);

...
export default function EditChecklistPage({ params }: EditChecklistPageProps) {
  ...
  useEffect(() => {
    ...
    const computedScope =
      (checklist.scope as ChecklistScope) ||
      (checklist.template?.scope as ChecklistScope) ||
      "general";
    const seeds =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim()) {
      toast({
        ...
        description: "Add a name so the team recognizes this checklist.",
        variant: "destructive",
      });
    ...
    if (validItems.length === 0) {
      toast({
        ...
        description: "Every checklist needs something to do.",
        variant: "destructive",
      });
      setActiveStep("items");
  ...
  const checklistItems = (
    <Card id="items">
      <CardHeader>
        ...
        <CardDescription>
          Organize every task that needs to be completed. Group items by category, assign owners,
  ...
  const handleSaveAsTemplate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "You can now reuse this checklist anytime.",
      });
    } catch (err) {
      toast({
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6">
      <div className="mb-8 space-y-4">
        <Button
          variant="ghost"
          onClick={() => router.push(`/org/${params.slug}/checklists`)}
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-2 text-base">
            Update tasks, timing, or ownership with the same guided builder you use when creating a
            checklist.
          </p>
          {isTemplateBased && (
            <p className="text-sm text-muted-foreground mt-2">
              Originally created from <strong>{checklist?.template?.title || "a template"}</strong>.
              Changes here only affect this checklist.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
        <form onSubmit={handleSubmit} className="space-y-6 order-1">
          <Card id="details">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={scope}
                  onValueChange={(value) => setScope(value as ChecklistScope)}
                  required
                >
                  <SelectTrigger>
          ...
          <div
            id="review"
            className="flex flex-col gap-3 border rounded-xl p-4 md:flex-row md:items-center md:justify-between"
          >
            <div>
              <p className="font-semibold">Ready to save your changes?</p>
              <p className="text-sm text-muted-foreground">
                Updates go live immediately. Save as a template to reuse this checklist elsewhere.
              </p>
            </div>

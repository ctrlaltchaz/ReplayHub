...
async function demonstrateRLS() {
  ...
  try {
    ...
    await prisma.$executeRawUnsafe(`SELECT set_config('app.tenant_id', '${alphaId}', false)`);
    const crossTenant1 =
      await prisma.$queryRaw`SELECT COUNT(*) as count FROM org_users WHERE tenant_id = ${testorgId}`;
    ...
    await prisma.$executeRawUnsafe(`SELECT set_config('app.tenant_id', '${testorgId}', false)`);
    const crossTenant2 =
      await prisma.$queryRaw`SELECT COUNT(*) as count FROM org_users WHERE tenant_id = ${alphaId}`;

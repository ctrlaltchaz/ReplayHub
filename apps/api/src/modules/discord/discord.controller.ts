...
export class DiscordController {
   ...
   */
  @Post('link')
  async linkServer(@TenantId() tenantId: string, @Body() dto: LinkDiscordServerDto) {
    // Validate with Zod
    const validated = LinkDiscordServerSchema.parse(dto);
    return this.discordService.linkServer(
      tenantId,
      validated,
      null,
      // org user context available on req.orgUser
      // controller lacks req; this endpoint currently only uses tenant-level guard without orgUser
      // keep actor null to avoid FK issues
      null
    );
  }
   ...
   */
  @Put('channels')
  async updateChannels(@TenantId() tenantId: string, @Body() dto: UpdateDiscordChannelsDto) {
    // Validate with Zod
    const validated = UpdateDiscordChannelsSchema.parse(dto);
    return this.discordService.updateChannels(tenantId, validated, null, null);
  }
   ...
   */
  @Put('settings')
  async updateSettings(@TenantId() tenantId: string, @Body() dto: UpdateDiscordSettingsDto) {
    // Validate with Zod
    const validated = UpdateDiscordSettingsSchema.parse(dto);
    return this.discordService.updateSettings(tenantId, validated, null, null);
  }
  ...
  async unlinkServer(@TenantId() tenantId: string) {
    await this.discordService.unlinkServer(tenantId, null, null);
  }
  ...
  async sendTestNotification(@TenantId() tenantId: string) {
    return this.discordService.sendTestNotification(tenantId, null, null);
  }
  ...
  async getChannels(@TenantId() tenantId: string) {
    ...
    console.log(
      `[DiscordController] Returning ${channels.length} channels to frontend:`,
      JSON.stringify(channels.slice(0, 3))
    );
    return channels;
  }

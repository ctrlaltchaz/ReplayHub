...
import { AssignRoleDto, CreateRoleDto, RemoveRoleDto, RoleService, UpdateRoleDto } from './services/role.service';
...
export class RbacController {
    ...
    async createRole(@Body() createRoleDto: CreateRoleDto, @Req() req: Request) {
        const role = await this.roleService.createRole(req.tenant!.id, createRoleDto);
        return { role };
    }
    ...
    async updateRole(
        ...
        const role = await this.roleService.updateRole(req.tenant!.id, roleId, updateRoleDto);
        return { role };
    }
    ...
    async deleteRole(@Param('id') roleId: string, @Req() req: Request) {
        const result = await this.roleService.deleteRole(req.tenant!.id, roleId);
        return result;
    }
    ...
    async seedRoles(@Req() req: Request) {
        ...
        await this.permissionService.seedDefaultPermissions(req.tenant!.id);

...
export function CreateTemplateDialog({ open, onOpenChange, orgSlug }: CreateTemplateDialogProps) {
    const { toast } = useToast();
    const createTemplate = useCreateChecklistTemplate(orgSlug);
    ...
    const handleSubmit = async () => {
        ...
        try {
            await createTemplate.mutateAsync({ title: title.trim(), scope, itemsJson: validItems });
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <DialogFooter>
                    ...
                    <Button onClick={handleSubmit} disabled={createTemplate.isPending}>{createTemplate.isPending ? 'Creating...' : 'Create Template'}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );

...
import { LinkDiscordServerDto, UpdateDiscordChannelsDto, UpdateDiscordSettingsDto } from './dto';
...
export class DiscordService {
  ...
  async linkServer(
    ...
    if (dto.botToken) {
      ...
      if (!verification.valid) {
        const errorMsg =
          verification.error ||
          'Invalid bot token. Please check the token is correct and the bot has proper intents enabled.';
      ...
      if (dto.guildId && verification.guilds) {
        ...
        if (!guild) {
          throw new Error(
            `Bot is not a member of guild ${dto.guildId}. Please invite the bot to your Discord server first.`
          );
        }
      ...
      this.logger.log(
        `Bot verified: ${verification.botTag}, guilds: ${verification.guilds?.length || 0}`
      );
    }
    ...
    await this.auditService.log({
      ...
      description: 'Linked Discord server',
      metadata: {
        guildId: dto.guildId,
        guildName,
        actorEmail,
  ...
  async updateChannels(
    ...
    try {
      ...
      await this.auditService.log({
        ...
        entityId: tenantId,
        orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
        description: 'Updated Discord channels',
        metadata: {
          channelEvents: dto.channelEvents,
          channelMatches: dto.channelMatches,
          channelRoster: dto.channelRoster,
          channelIncidents: dto.channelIncidents,
          channelGeneral: dto.channelGeneral,
          actorEmail,
        },
      });
  ...
  async updateSettings(
    ...
    try {
      ...
      await this.auditService.log({
        ...
        entityId: tenantId,
        orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
        description: 'Updated Discord settings',
        metadata: {
          settings: dto,
          actorEmail,
        },
   ...
   */
  async unlinkServer(tenantId: string, actorOrgUserId?: string | null, actorEmail?: string | null) {
    ...
    await this.auditService.log({
      ...
      entityId: tenantId,
      orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
      description: 'Disconnected Discord integration',
      metadata: { actorEmail },
    });
  ...
  async sendTestNotification(
    ...
    const results = await Promise.all(
      channelsToTest.map(async channel => {
        const success = await this.botService.postToChannel(tenantId, channel.id, {
    ...
    await this.auditService.log({
      ...
      entityId: tenantId,
      orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
      description: 'Sent Discord test notification',
      metadata: {
        channelsTested: channelsToTest.map(ch => ch.id),
  ...
  async notifyEvent(
    ...
    try {
      ...
      const success = await this.botService.postToChannel(tenantId, config.channelEvents, {
      ...
      await this.logNotification(tenantId, 'event', 'channel', 'failed', error.message, {
        event,
        type,
      });
    }
  ...
  async notifyMatch(
    ...
    try {
      ...
      const success = await this.botService.postToChannel(tenantId, config.channelMatches, {
      ...
      await this.logNotification(tenantId, 'match', 'channel', 'failed', error.message, { match });
    }
  }
  ...
  async notifyRoster(
    ...
    try {
      ...
      const success = await this.botService.postToChannel(tenantId, config.channelRoster, {
        title: `${emoji} ${title}`,
        description,
        color,
        fields,
      ...
      await this.logNotification(tenantId, 'roster', 'channel', 'failed', error.message, {
        update,
      });
    }
  }
  ...
  async notifyIncident(
    ...
    try {
      ...
      const success = await this.botService.postToChannel(tenantId, config.channelIncidents, {
      ...
      await this.logNotification(tenantId, 'incident', 'channel', 'failed', error.message, {
        incident,
      });
    }
  }
  ...
  async sendUserDM(
    ...
    try {
      ...
      await this.logNotification(tenantId, 'dm', 'bot_dm', 'failed', error.message, {
        discordUserId,
        message,
      });
      return false;
  ...
  async linkUserDiscord(globalUserId: string, code: string, redirectUri: string) {
    try {
      ...
      try {
        ...
        if (botTenantId) {
          await this.botService.sendDirectMessage(botTenantId, discordUser.id, {
            title: '🎉 Discord Account Linked!',
            description:
              'Your Discord account has been successfully linked to your ReplayHub account.',
            color: 0x5865f2,
            fields: [
              {
                name: '✅ What this means',
                value:
                  'You can now receive direct message notifications for:\n• Event updates and reminders\n• Match schedules\n• Team roster changes\n• Important announcements',
                inline: false,
              },
              {
                name: '⚙️ Manage Notifications',
                value:
                  'You can customize your notification preferences in your profile settings at any time.',
                inline: false,
              },
            ],
          });
        ...
        this.logger.warn(
          `Could not send welcome DM to ${discordUser.username}: ${dmError.message}`
        );
  ...
  async getGlobalUserIds(orgUserIds: string[]): Promise<string[]> {
    ...
    return orgUsers.map(ou => ou.globalUserId).filter((id): id is string => id !== null);
  }
}

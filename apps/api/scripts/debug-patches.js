...
async function debugPatches() {
  ...
  try {
    ...
    try {
      console.log(
        `Testing: GET /api/org/testorg/players/availability?date=2024-03-15&teamId=${teamId}`
      );
    ...
    try {
      ...
      // Test setting slots
      try {
        const slotsResponse = await client.post(
          `/api/org/testorg/lineups/${lineupResponse.data.id}/slots`,
          {
            slots: [
              {
                playerId: playerId,

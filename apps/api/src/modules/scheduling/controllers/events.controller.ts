...
export class EventsController {
  ...
  async createEvent(
    ...
    return await this.eventsService.createEvent(
      ...
      globalUser.id,
      createEventDto,
      req.orgUser?.email ?? globalUser?.email ?? null
    );
  }
  ...
  @Can('events.view')
  async findEvents(@Req() req: any, @Query() queryDto: QueryEventsDto) {
    const tenantId = req.tenant?.id;
    if (!tenantId) {
      throw new ConflictException('Tenant context required');
    }
  ...
  @Can('events.view')
  async findEventById(@Req() req: any, @Param('id') id: string) {
    const tenantId = req.tenant?.id;
    if (!tenantId) {
      throw new ConflictException('Tenant context required');
    }
  ...
  async deleteEvent(@Req() req: any, @Param('id') id: string) {
    ...
    const result = await this.eventsService.deleteEvent(
      tenantId,
      id,
      req.orgUser?.id ?? req.globalUser?.id ?? null,
      req.orgUser?.email ?? req.globalUser?.email ?? null
  ...
  async assignLineup(
    ...
    const event = await this.eventsService.assignLineup(
      tenantId,
      eventId,
      assignLineupDto.lineupId,
      req.orgUser?.id ?? req.globalUser?.id ?? null,
      req.orgUser?.email ?? req.globalUser?.email ?? null
    );
    if (!event) {
...
export class CalendarController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves events for a calendar week with timezone support and flexible date ranges. Defaults to current Monday if no start date provided.',
  })
  @ApiResponse({
    status: 200,
  ...
  @Can('events.view')
  async getWeekView(@Req() req: any, @Query() queryDto: CalendarWeekQueryDto) {
    const tenantId = req.tenant?.id;
    if (!tenantId) {
      throw new ConflictException('Tenant context required');
    }
    ...
    try {
      ...
      if (error.message?.includes('Invalid timezone')) {
        throw new BadRequestException(
          'Invalid timezone identifier. Please use a valid IANA timezone (e.g., "America/New_York").'
        );
      }

...
export class OrgUserController {
    ...
    async updateUser(
        ...
        const user = await this.orgUserService.updateUser(req.tenant!.id, userId, updateUserDto);
        return { user };
    }
    ...
    async assignRoles(
        ...
        const user = await this.orgUserService.assignRoles(req.tenant!.id, userId, assignRolesDto);
        return { user };
    }
    ...
    async deactivateUser(@Param('id') userId: string, @Req() req: Request) {
        const user = await this.orgUserService.deactivateUser(req.tenant!.id, userId);
        return { user };
    }
    ...
    async reactivateUser(@Param('id') userId: string, @Req() req: Request) {
        const user = await this.orgUserService.reactivateUser(req.tenant!.id, userId);
        return { user };
    }
    ...
    async deleteUser(@Param('id') userId: string, @Req() req: Request) {
        await this.orgUserService.deleteUser(req.tenant!.id, userId);

        ...
        return (
            <Card className="hover:shadow-lg transition-shadow">
                <CardHeader>
                    ...
                    <CardDescription>
                        No player profile associated with your account
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <p className="text-sm text-muted-foreground">
                        Ask an admin to create a player profile and link it to your account to see your upcoming events.
                    </p>
                </CardContent>
            </Card>
        );
    ...
    if (!nextEvent) {
        return (
            <Card className="hover:shadow-lg transition-shadow">
                ...
                <CardContent>
                    <p className="text-sm text-muted-foreground">
                        You're currently on {playerInfo.teams.length} team{playerInfo.teams.length !== 1 ? 's' : ''}, but no events are scheduled yet.
                    </p>
                </CardContent>
            </Card>
        );
        ...
        <Card className="hover:shadow-lg transition-shadow border-primary/20">
            ...
            <CardContent className="space-y-4">
                ...
                <div>
                    ...
                    <Badge className={`mt-2 ${eventColorClass}`}>
                        {nextEvent.eventType}
                    </Badge>
                </div>
                ...
                {nextEvent.callTime && (
                    <div className="flex items-center gap-2 text-sm bg-primary/10 p-2 rounded-md">
                        ...
                        <span>
                            <span className="font-semibold">Call Time:</span> {format(new Date(nextEvent.callTime), "h:mm a")}
                        </span>
                    </div>
                )}
                ...
                <div className="pt-3 border-t">
                    <p className="text-xs text-muted-foreground">
                        ...
                        {playerInfo.teams && playerInfo.teams.length > 0 && playerInfo.teams[0].team && (
                            <> on <span className="font-semibold text-foreground">{playerInfo.teams[0].team.name}</span></>
                        )}
                    </p>
                </div>

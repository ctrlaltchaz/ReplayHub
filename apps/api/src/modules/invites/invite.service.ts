...
export class InviteService {
    ...
    async acceptInvite(
        ...
        if (!orgSlugOrTenantId.startsWith('cmg')) { // Not a CUID, assume it's a slug
            const org = await this.prisma.organisation.findUnique({
    ...
    private validatePasswordStrength(password: string) {
        ...
        if (errors.length > 0) {
            throw new BadRequestException(`Password does not meet complexity requirements: ${errors.join('; ')}`);
        }
    }
}
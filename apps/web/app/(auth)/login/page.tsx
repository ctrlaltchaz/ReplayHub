...
function UniversalLoginContent() {
  ...
  const loginResponseHasGlobal =
    loginResponse?.user?.hasGlobalAccount ??
    (loginResponse?.userType === "global" || loginResponse?.userType === "both");
  const shouldShowOrgSelection = Boolean(
    !requiresTotp && loginResponse && membershipOptions.length > 1 && !loginResponseHasGlobal
  );
  ...
  const handleTotpVerify = async (e: React.FormEvent) => {
    ...
    try {
      const verifyData: TotpVerifyRequest = {
        token: totpCode,
        userType:
          (loginResponse.user?.hasGlobalAccount ?? loginResponse.userType === "global")
            ? "global"
            : "org",
      ...
      if (err.message?.toLowerCase().includes("expired")) {
        errorMessage =
          "The code has expired. Please try with a new code from your authenticator app.";
  ...
  const handleLoginSuccess = async (data: UniversalLoginResponse) => {
    try {
      ...
      const globalOrganisations =
        data.user?.globalOrganisations ?? data.globalUser?.organizations ?? [];
        ...
        <div className="relative max-w-md w-full mx-4 space-y-8 animate-fade-in z-10">
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/30 backdrop-blur-sm border border-white/40 shadow-lg">
              ...
              <span className="text-xs font-medium text-white">Extra layer of security</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4 xl:space-y-5 text-white drop-shadow-lg">
            <div className="flex items-start gap-4">
              ...
              <p className="text-lg xl:text-xl font-medium">
                Unified platform for all esports operations
              </p>
            </div>
            <div className="flex items-start gap-4">
              <div className="w-3 h-3 rounded-full bg-white mt-2 flex-shrink-0"></div>

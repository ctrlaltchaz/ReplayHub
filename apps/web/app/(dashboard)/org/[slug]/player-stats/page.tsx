...
interface PlayerStats {
  ...
  stats?: {
    ...
    gameBreakdown?: Record<
      string,
      { totalKills?: number; totalDeaths?: number; totalAssists?: number; gamesPlayed?: number }
    >;
  };
}
...
export default function PlayerStatsPage() {
  ...
  const availableGames = Array.from(
    new Set(playersWithStats.flatMap((p) => p.teams?.map((t) => t.team.game) || []))
  )
  ...
  const availableTeams = Array.from(
    new Set(
      playersWithStats.flatMap(
        (p) => p.teams?.map((t) => JSON.stringify({ id: t.team.name, name: t.team.name })) || []
      )
    )
  )
    .map((t) => JSON.parse(t))
  ...
  const filteredPlayers = playersWithStats.filter((player) => {
    ...
    // Game filter
    const matchesGame =
      gameFilter === "all" || player.teams?.some((t) => t.team.game === gameFilter);

    // Team filter
    const matchesTeam =
      teamFilter === "all" || player.teams?.some((t) => t.team.name === teamFilter);
          ...
          filteredPlayers.map((player) => (
            ...
            >
              ...
              <CardContent>
                {player.teams && player.teams.length > 0 && (
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {player.teams.map((tm) => (
                        <span key={tm.id} className="text-xs bg-muted px-2 py-1 rounded">
                          {tm.team.name}
                        </span>
                      ))}
                    </div>
                ...
                {player.stats ? (
                  (() => {
                    const breakdown = player.stats.gameBreakdown || {};
                    const totals = Object.values(breakdown).reduce(
                      (acc, game: any) => ({
                        kills: acc.kills + (game.totalKills || 0),
                        deaths: acc.deaths + (game.totalDeaths || 0),
                      }),
                      { kills: 0, deaths: 0 }
                    );
                    return (
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <p className="text-xl font-semibold">{totals.kills}</p>
                          <p className="text-xs text-muted-foreground">Total Kills</p>
                        </div>
                        <div>
                          <p className="text-xl font-semibold">{totals.deaths}</p>
                          <p className="text-xs text-muted-foreground">Total Deaths</p>
                        </div>
                        <div>
                          <p className="text-xl font-semibold">{player.stats.totalGames}</p>
                          <p className="text-xs text-muted-foreground">Games Played</p>
                        </div>
                        <div>
                          <p className="text-xl font-semibold text-primary">
                            {player.stats.totalMvps}
                          </p>

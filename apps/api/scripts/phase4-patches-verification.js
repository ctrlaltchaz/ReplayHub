...
async function testAvailabilityBulkQueries() {
  ...
  try {
    ...
    const availResponse = await makeRequest(
      `/api/org/${testOrg.slug}/availability?date=${today}&teamId=${testTeam.id}`
    );

    if (availResponse.ok) {
      const availData = await availResponse.json();
      console.log(
        `✅ PATCH 1 SUCCESS: Got availability data for team (${Array.isArray(availData) ? availData.length : 'unknown'} records)`
      );
      ...
      if (Array.isArray(availData) && availData.length > 0) {
        ...
        console.log(
          `  - Sample record: Player ${sample.player?.gamerTag || 'Unknown'}, Status: ${sample.status || 'null'}`
        );
      }
    } else {
...
async function testLineupMembershipWorkflow() {
  ...
  try {
    ...
    const setSlotsResponse1 = await makeRequest(
      `/api/org/${testOrg.slug}/lineups/${lineup.id}/slots`,
      {
        method: 'POST',
        body: JSON.stringify({
          slots: [
    ...
    if (setSlotsResponse1.status === 422) {
      ...
      console.log(
        `❌ PATCH 2a FAILED: Expected 422 but got ${setSlotsResponse1.status} - ${error}`
      );
    }

    // Test 2: Try to set slots with autoAttachMissing (should succeed)
    console.log('\n--- Testing with autoAttachMissing ---');
    const setSlotsResponse2 = await makeRequest(
      `/api/org/${testOrg.slug}/lineups/${lineup.id}/slots`,
      {
        method: 'POST',
        body: JSON.stringify({
...
async function testPublishImmutability() {
  ...
  try {
    ...
    const publishResponse = await makeRequest(
      `/api/org/${testOrg.slug}/lineups/${lineup.id}/publish`,
      {
        method: 'POST',
      }
    );
    ...
    if (publishResponse.ok) {
      ...
      console.log(
        `✅ PATCH 3a SUCCESS: Published lineup (published: ${publishedLineup.published})`
      );
    } else {
    ...
    const republishResponse = await makeRequest(
      `/api/org/${testOrg.slug}/lineups/${lineup.id}/publish`,
      {
        method: 'POST',
      }
    ...
    if (republishResponse.status === 409) {
      ...
      console.log(
        `❌ PATCH 3b FAILED: Expected 409 but got ${republishResponse.status} - ${error}`
      );
    }

    // Test 2: Try to modify published lineup (should fail with 409)
    console.log('\n--- Testing modification after publish ---');
    const modifyResponse = await makeRequest(
      `/api/org/${testOrg.slug}/lineups/${lineup.id}/slots`,
      {
        method: 'POST',
        body: JSON.stringify({
    ...
    if (modifyResponse.status === 409) {
      ...
      console.log(
        `✅ PATCH 3c SUCCESS: Got 409 error for modification after publish: ${error.message}`
      );
    } else {
...
async function main() {
  ...
  try {
    ...
    console.log(
      '2. 👥 Lineup membership workflow - Added autoAttachMissing with proper 422 error codes'
    );
    console.log(
      '3. 🔒 Publish immutability - Enforced with 409 conflicts for post-publish modifications'
    );
  } catch (error) {

...
export class GlobalAuthController {
  ...
  async updateProfile(@Body() updateProfileDto: UpdateProfileDto, @Req() req: Request) {
    try {
      const user = await this.globalAuthService.updateProfile(
        req.session.userId!,
        updateProfileDto
      );
      return user;
  ...
  async requestEmailChange(@Body() changeEmailDto: ChangeEmailDto, @Req() req: Request) {
    try {
      const result = await this.globalAuthService.requestEmailChange(
        req.session.userId!,
  ...
  // Quick Login PIN Endpoints
  @UseGuards(SessionGuard)
  @Post('quick-login/setup')
  async setupQuickLoginPin(
    @Body() body: { pin: string; currentPassword: string; deviceId: string },
    @Req() req: Request
  ) {
    try {
      const result = await this.globalAuthService.setupQuickLoginPin(
        req.session.userId!,
        body.pin,
  ...
  @Post('quick-login/verify')
  async verifyQuickLoginPin(@Body() body: { pin: string; email: string }, @Req() req: Request) {
    try {
      const user = await this.globalAuthService.verifyQuickLoginPin(body.pin, body.email);
      ...
      return { message: 'Quick login successful', user };
    } catch (error) {
      throw new UnauthorizedException(error.message);
    }
  }
  ...
  @UseGuards(SessionGuard)
  @Post('quick-login/disable')
  async disableQuickLoginPin(@Body() body: { currentPassword: string }, @Req() req: Request) {
    try {
      const result = await this.globalAuthService.disableQuickLoginPin(
        req.session.userId!,
        body.currentPassword
      );
      return result;
  ...
  @Get('quick-login/check')
  async checkQuickLoginAvailable(@Req() req: Request) {
    try {
      const email = req.query.email as string;
      if (!email) {
        throw new BadRequestException('Email is required');
      }
      const result = await this.globalAuthService.checkQuickLoginAvailable(email);
      return result;
    } catch (error) {
      throw new BadRequestException(error.message);
    }

...
class Phase6Verifier {
  ...
  async testRunsheetWorkflow() {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `✅ Runsheet created: ${createResponse.status}, {id: "${createResponse.data.id}", status: "${createResponse.data.status}"}`
        );
      } else {
        console.log(
          `❌ Runsheet creation failed: ${createResponse.status}, ${JSON.stringify(createResponse.data)}`
        );
        return false;
      }
      ...
      if (
        ...
        console.log(
          `✅ Items added: ${itemsResponse.status}, count = ${itemsResponse.data.length}`
        );
      } else {
        console.log(
          `❌ Items creation failed: ${itemsResponse.status}, ${JSON.stringify(itemsResponse.data)}`
        );
        return false;
      }
      ...
      if (
        ...
        console.log(
          `✅ Runsheet approved: ${approveResponse.status}, status: "${approveResponse.data.status}", revision: ${approveResponse.data.revision}`
        );
      } else {
        console.log(
          `❌ Approval failed: ${approveResponse.status}, ${JSON.stringify(approveResponse.data)}`
        );
        return false;
      }
      ...
      if ([200, 201].includes(lockResponse.status) && lockResponse.data.status === 'locked') {
        ...
        console.log(
          `✅ Runsheet locked: ${lockResponse.status}, status: "${lockResponse.data.status}"`
        );
      } else {
        console.log(
          `❌ Locking failed: ${lockResponse.status}, ${JSON.stringify(lockResponse.data)}`
        );
        return false;
      }
      ...
      const editResponse = await this.session.put(
        `/runsheets/${this.testData.runsheetId}/items/${itemsResponse.data[0].id}`,
        {
      ...
      if (
        ...
        console.log(
          `✅ Locked edit blocked: ${editResponse.status}, error: "${editResponse.data.error}"`
        );
      } else {
        console.log(
          `❌ Locked edit not properly blocked: ${editResponse.status}, ${JSON.stringify(editResponse.data)}`
        );
        return false;
      }
  ...
  async testChecklistWorkflow() {
    ...
    try {
      ...
      if (templateResponse.status === 201 && templateResponse.data.version === 1) {
        ...
        console.log(
          `✅ Template created: ${templateResponse.status}, version: ${templateResponse.data.version}`
        );
      } else {
        console.log(
          `❌ Template creation failed: ${templateResponse.status}, ${JSON.stringify(templateResponse.data)}`
        );
        return false;
      }
      ...
      if (
        ...
        console.log(
          `✅ Checklist created: ${checklistResponse.status}, links template + assigneeId`
        );
      } else {
        console.log(
          `❌ Checklist creation failed: ${checklistResponse.status}, ${JSON.stringify(checklistResponse.data)}`
        );
        return false;
      }
      ...
      if (runResponse.status === 201 && runResponse.data.runnerId && runResponse.data.createdAt) {
        ...
        console.log(
          `✅ Checklist run by assignee: ${runResponse.status}, includes runnerId and timestamp`
        );
      } else {
        console.log(
          `❌ Checklist run failed: ${runResponse.status}, ${JSON.stringify(runResponse.data)}`
        );
        return false;
      }
      ...
      if (unauthorizedRunResponse.status === 403) {
        ...
        console.log(
          `❌ Unauthorized run not blocked: ${unauthorizedRunResponse.status}, ${JSON.stringify(unauthorizedRunResponse.data)}`
        );
        return false;
      }
  ...
  async testSecurity() {
    ...
    try {
      ...
      const allTablesHavePolicies = expectedTables.every(table =>
        tablesWithPolicies.includes(table)
      );
      const allPoliciesHaveTenantFilter = policies.every(
        p => p.qual && p.qual.includes('tenant_id')
      );

      if (allTablesHavePolicies && allPoliciesHaveTenantFilter) {
        this.results.rlsPolicies = '✅';
        console.log(
          `✅ RLS policies active: ${tablesWithPolicies.length}/5 tables with tenant_id filtering`
        );
      } else {
        console.log(
          `❌ RLS policies incomplete: ${tablesWithPolicies.length}/5 tables, tenant filtering: ${allPoliciesHaveTenantFilter}`
        );
        console.log(
          'Policies:',
          policies.map(p => `${p.tablename}: ${p.polname}`)
        );
        return false;
      }
  ...
  async testIntegrity() {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `✅ Runsheet integrity: items array (len=${runsheetResponse.data.items.length}), status: "${runsheetResponse.data.status}"`
        );
      } else {
        console.log(
          `❌ Runsheet integrity failed: ${runsheetResponse.status}, items: ${runsheetResponse.data.items?.length}, status: ${runsheetResponse.data.status}`
        );
        return false;
      }
      ...
      if (
        ...
        console.log(
          `✅ Checklist runs: list shows ${runsResponse.data.length} entry with pass/fail breakdown`
        );
      } else {
        console.log(
          `❌ Checklist runs failed: ${runsResponse.status}, runs: ${runsResponse.data?.length}`
        );
        return false;
      }
      ...
      if (allHaveTenantId && allHaveTimestamps) {
        ...
        console.log(
          `❌ Database consistency failed: tenantId: ${allHaveTenantId}, timestamps: ${allHaveTimestamps}`
        );
        return false;
      }
  ...
  async generateSummary() {
    ...
    console.log(
      `| Runsheet approval | 200, status:approved, revision:2 | ${this.results.runsheetApproval} |`
    );
    ...
    console.log(
      `| Checklist creation | 201, links template+assignee | ${this.results.checklistCreation} |`
    );
    ...
    console.log(
      `| Runsheet integrity | items array + locked status | ${this.results.runsheetIntegrity} |`
    );

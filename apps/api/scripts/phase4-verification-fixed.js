...
function logTest(category, subcategory, status, message, details = {}) {
  ...
  if (details.payload && typeof details.payload === 'object') {
    console.log(
      `   Payload: ${JSON.stringify(details.payload, null, 2).substring(0, 300)}${JSON.stringify(details.payload, null, 2).length > 300 ? '...' : ''}`
    );
  }
}
...
async function testAvailabilityManagement() {
  ...
  try {
    ...
    try {
      await client.post(`${API_BASE}/org/${TENANT_SLUG}/players/nonexistent123/availability`, {
        date: '2025-10-16',
        status: 'available',
      });
      ...
      try {
        ...
        if (error.response?.status === 403 || error.response?.status === 409) {
          logTest(
            ...
            'immutability',
            '✅',
            'Correctly prevented modification after publish',
            {
              responseCode: error.response?.status,
              payload: error.response?.data,
            }
          ...
          logTest(
            ...
            'immutability',
            '❌',
            'Unexpected response for post-publish modification',
            {
              responseCode: error.response?.status,
              payload: error.response?.data,
            }
      ...
      if (testData.eventId) {
        ...
        logTest('pdf', 'generation', '✅', 'Call sheet generated', {
          ...
          payload: {
            ...
            contentLength:
              callSheetResponse.headers['content-length'] || callSheetResponse.data.byteLength,
          },
        });
      }
    } catch (error) {
...
async function testRLSEnforcement() {
  ...
  try {
    ...
    if (availResponse.data.length >= 0) {
      // Should return tenant-specific data only
  ...
  if (!overallPass) {
    ...
    categories.forEach(category => {
      Object.keys(results[category]).forEach(subcategory => {
        ...
        if (result.status === '❌') {
          console.log(
            `- ${category}.${subcategory}: ${result.details[result.details.length - 1]?.message || 'Failed'}`
          );
        }
      });
    });

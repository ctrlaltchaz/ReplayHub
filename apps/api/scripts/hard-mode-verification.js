...
class HardModeVerifier {
    ...
    async test1_IndexOrderingIntegrity() {
        ...
        try {
            ...
            console.log(`   Duplicate idx test: ${duplicateTest ? '✅' : '❌'} (${duplicateResponse.status})`);

            // Try to reorder by updating indices
            const items = itemsResponse.data;
            const reorderResponse1 = await this.session.put(`/runsheets/${runsheetId}/items/${items[0].id}`, { idx: 2 });
            const reorderResponse2 = await this.session.put(`/runsheets/${runsheetId}/items/${items[2].id}`, { idx: 0 });

            const reorderTest = reorderResponse1.status === 200 && reorderResponse2.status === 200;
            console.log(`   Reorder test: ${reorderTest ? '✅' : '❌'} (${reorderResponse1.status}, ${reorderResponse2.status})`);
    ...
    async test2_RevisionAuditIntegrity() {
        ...
        try {
            ...
            const doubleApprovalPrevented = approve2.status === 409 || approve2.data.revision === approve1.data.revision;
            console.log(`   Second approval: ${approve2.status} (revision: ${approve2.data?.revision || 'N/A'})`);
    ...
    async test6_PermissionsMatrix() {
        ...
        try {
            ...
            console.log(`   Admin can create: ${adminCanCreate ? '✅' : '❌'} (${adminCreateResponse.status})`);
    ...
    async test8_Concurrency() {
        ...
        try {
            ...
            const oneLockSucceeded = (lock1.status === 200 && lock2.status === 409) ||
            ...
            const oneAddSucceeded = (add1.status === 201 && add2.status === 409) ||
    ...
    async test9_PaginationFiltering() {
        ...
        try {
            ...
            console.log(`   Runsheets list: ${hasRunsheets ? '✅' : '❌'} (${runsheetsList.status}, ${runsheetsList.data?.length || 0} items)`);
        ...
        try {
            ...
            if (fs.existsSync(migrationDir)) {
                ...
                migrationFiles = migrations.filter(m =>
                    fs.readFileSync(`${migrationDir}/${m}/migration.sql`, 'utf8')
                        .includes('runsheets') ||
                    fs.readFileSync(`${migrationDir}/${m}/migration.sql`, 'utf8')
                        .includes('checklist')
                );
    ...
    async generateSummary() {
        ...
        const tests = [
            { name: 'Index + Ordering Integrity', key: 'indexOrdering', evidence: 'Duplicate idx → 409, reorder successful' },
            { name: 'Revision + Audit Integrity', key: 'revisionAudit', evidence: 'Double approval prevented/idempotent' },
            { name: 'Lock Bypass Attempts', key: 'lockBypass', evidence: 'All locked operations → 409' },
            { name: 'Template Versioning', key: 'templateVersioning', evidence: 'C1=3 items, C2=4 items from different versions' },
            { name: 'Checklist Run Idempotency', key: 'runIdempotency', evidence: 'Multiple runs allowed, invalid idx → 400' },
            { name: 'Permissions Matrix', key: 'permissionsMatrix', evidence: 'Admin operations successful' },
            { name: 'RLS Bypass Attempts', key: 'rlsBypass', evidence: 'Cross-tenant → 403/404, all tables have RLS' },
            { name: 'Concurrency Handling', key: 'concurrency', evidence: 'Parallel operations properly serialized' },
            { name: 'Pagination + Filtering', key: 'paginationFiltering', evidence: 'Lists work, filtering by eventId works' },
            { name: 'Input Validation', key: 'inputValidation', evidence: 'Invalid inputs → 400+ status codes' },
            { name: 'Backup + Migration Safety', key: 'backupMigration', evidence: 'Migration files exist, indexes created' }
        ];

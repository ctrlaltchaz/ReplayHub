...
export function Sidebar({
  ...
  return (
    <>
      {/* Mobile overlay */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-black/50 z-40 lg:hidden" onClick={onMobileMenuClose} />
      )}
        ...
        <nav className="flex-1 overflow-y-auto space-y-2 p-4">
          {globalUser && !slug ? (
            ...
            <>
              ...
              {isGlobalAdmin(globalUser) && (
                <>
                  <div className={cn("border-t pt-4 mt-4", isCollapsed && "border-t-0 pt-0 mt-0")}>
                    {!isCollapsed && (
            ...
            <>
                    ...
                    <div key={item.key}>
                      {hasChildren ? (
                        ...
                        >
                          ...
                          {!isCollapsed && (
                            <>
                              <span className="flex-1 truncate font-medium font-montserrat">
                                {item.label}
                              </span>
                              <ChevronDown
                                className={cn(
              ...
              {(() => {
                ...
                const hasAccess =
                  isLoadingOrg || isStillLoading ? true : checkPermission(settingsItem.required);
            ...
            {isAdminPage ? (
              ...
              <>
                {ORG_NAV.filter((item) => item.key !== "settings").map((item) => {
                  ...
                  const hasAccess =
                    isLoadingOrg || isStillLoading ? true : checkPermission(item.required);
                        ...
                        <div className="ml-9 mt-1 space-y-1">
                          {item.children.map((child) => {
                            const childHasAccess =
                              isLoadingOrg || isStillLoading
                                ? true
                                : checkPermission(child.required);
                            if (!childHasAccess) return null;

...
class Phase6VerificationTests {
    ...
    async testA_RunsheetWorkflow() {
        ...
        try {
            ...
            if (createResponse.status === 201 && createResponse.data.id && createResponse.data.status === 'draft') {
                ...
                console.log(`✅ ${createResponse.status} with {id: "${createResponse.data.id}", title: "${createResponse.data.title}", status: "${createResponse.data.status}"}\n`);
            } else {
                console.log(`❌ Expected 201 with draft status, got ${createResponse.status}: ${JSON.stringify(createResponse.data)}\n`);
                return false;
            }
            ...
            const itemsResponse = await this.session.post(`/runsheets/${this.testData.runsheetId}/items`, {
                items: [
                    { idx: 0, title: 'Setup Equipment', ownerId: this.testData.currentUserId, durationMs: 900000, notes: 'Configure all streaming gear' },
                    { idx: 1, title: 'Player Registration', durationMs: 600000, notes: 'Check-in all participants' },
            ...
            if (itemsResponse.status === 201 && Array.isArray(itemsResponse.data) && itemsResponse.data.length === 3) {
                ...
                console.log(`❌ Expected 201 with 3 items, got ${itemsResponse.status}: ${JSON.stringify(itemsResponse.data)}\n`);
                return false;
            }
            ...
            if ([200, 201].includes(approveResponse.status) &&
                ...
                console.log(`✅ ${approveResponse.status}, status: "${approveResponse.data.status}", revision: ${approveResponse.data.revision}\n`);
            } else {
                console.log(`❌ Expected 200 with approved/revision:2, got ${approveResponse.status}: ${JSON.stringify(approveResponse.data)}\n`);
                return false;
            }
            ...
            if ([200, 201].includes(lockResponse.status) && lockResponse.data.status === 'locked') {
                ...
                console.log(`❌ Expected 200 with locked status, got ${lockResponse.status}: ${JSON.stringify(lockResponse.data)}\n`);
                return false;
            }
            ...
            const editResponse = await this.session.put(`/runsheets/${this.testData.runsheetId}/items/${this.testData.itemIds[0]}`, {
            ...
            if (editResponse.status === 409 && editResponse.data.error && editResponse.data.error.toLowerCase().includes('locked')) {
                ...
                console.log(`❌ Expected 409 with locked error, got ${editResponse.status}: ${JSON.stringify(editResponse.data)}\n`);
                return false;
            }
    ...
    async testB_ChecklistWorkflow() {
        ...
        try {
            ...
            if (templateResponse.status === 201 && templateResponse.data.version === 1) {
                ...
                console.log(`❌ Expected 201 with version:1, got ${templateResponse.status}: ${JSON.stringify(templateResponse.data)}\n`);
                return false;
            }
            ...
            if (checklistResponse.status === 201 &&
                ...
                console.log(`❌ Expected 201 with template link, got ${checklistResponse.status}: ${JSON.stringify(checklistResponse.data)}\n`);
                return false;
            }
            ...
            if (runResponse.status === 201 && runResponse.data.runnerId && runResponse.data.createdAt) {
                ...
                console.log(`✅ ${runResponse.status}, includes runnerId: "${runResponse.data.runnerId}" and timestamp: "${runResponse.data.createdAt}"\n`);
            } else {
                console.log(`❌ Expected 201 with runnerId/timestamp, got ${runResponse.status}: ${JSON.stringify(runResponse.data)}\n`);
                return false;
            }
            ...
            if (unauthorizedRunResponse.status === 403) {
                ...
                console.log(`❌ Expected 403 for non-assignee, got ${unauthorizedRunResponse.status}: ${JSON.stringify(unauthorizedRunResponse.data)}\n`);
                return false;
            }
    ...
    async testD_IntegrityReports() {
        ...
        try {
            ...
            if (runsheetResponse.status === 200 &&
                ...
                console.log(`✅ Returns items array (len=${runsheetResponse.data.items.length}), status:"${runsheetResponse.data.status}"\n`);
            } else {
                console.log(`❌ Expected items array len=3 + locked status, got: status=${runsheetResponse.data.status}, items=${runsheetResponse.data.items?.length}\n`);
                return false;
            }
            ...
            if (runsResponse.status === 200 &&
                ...
                console.log(`❌ Expected 1 run entry with breakdown, got: ${runsResponse.data?.length} entries\n`);
                return false;
            }
            ...
            if (hasValidTimestamps && hasConsistentData) {
                ...
                console.log(`❌ Inconsistent data: timestamps=${hasValidTimestamps}, tenantId=${hasConsistentData}\n`);
                return false;
            }
    ...
    generateSummaryTable(results) {
        ...
        console.log(`| Runsheet integrity | items array + locked | ${results.runsheetIntegrity ? '✅' : '❌'} |`);
        console.log(`| Checklist runs | 1 entry with breakdown | ${results.checklistRuns ? '✅' : '❌'} |`);
        console.log(`| Database consistency | timestamps + tenantId | ${results.dbConsistency ? '✅' : '❌'} |`);
        ...
        if (passedTests === totalTests) {
            console.log('🎉 **Phase 6 verified: runsheets + checklists fully functional with proper tenant isolation, permissions, and locking.**');
        } else {
...
async function runPhase6Verification() {
    ...
    try {
        ...
        results.runsheetIntegrity = results.checklistRuns = results.dbConsistency = await verifier.testD_IntegrityReports();

...
export class PlayerController {
  ...
  @Post()
  @Can('player.create')
  async create(@Req() req: Request, @Body() createPlayerDto: CreatePlayerDto) {
  ...
  @Get()
  @Can('roster.view')
  async findAll(@Req() req: Request, @Query() query: PlayerQueryDto) {
    return this.playerService.findMany(req.tenant!.id, query);
  }

  @Get('availability')
  @Can('availability.manage')
  async getAvailability(@Req() req: Request, @Query() query: AvailabilityQueryDto) {
    return this.playerService.getAvailability(req.tenant!.id, query.date, query.teamId);
  }
  ...
  async delete(@Req() req: Request, @Param('id') id: string) {
    return this.playerService.delete(
      req.tenant!.id,
      id,
      req.orgUser?.id,
      req.orgUser?.email ?? req.globalUser?.email ?? null
  ...
  async linkToUser(
    ...
    return this.playerService.linkToUser(
      req.tenant!.id,
      id,
      linkData,
      req.orgUser?.id,
  ...
  @Delete(':id/link-user')
  @Can('player.link_user')
  async unlinkFromUser(@Req() req: Request, @Param('id') id: string) {
    return this.playerService.unlinkFromUser(
      req.tenant!.id,
      id,
      req.orgUser?.id,
      req.orgUser?.email ?? req.globalUser?.email ?? null
  ...
  async syncUserData(@Req() req: Request, @Param('id') playerId: string) {
    const globalUserId = req.globalUser?.id;
    if (!globalUserId) {
      throw new Error('User not authenticated');
    }
    // Sync user data to player profile
    return this.playerService.syncUserDataToPlayer(req.tenant!.id, playerId, globalUserId);
  }

  @Get(':id/game-stats')
  @Can('roster.view')
  async getPlayerGameStats(@Req() req: Request, @Param('id') playerId: string) {
    const globalUserId = req.globalUser?.id;
    const stats = await this.playerService.getPlayerGameStats(
      req.tenant!.id,
      playerId,
      globalUserId
    );

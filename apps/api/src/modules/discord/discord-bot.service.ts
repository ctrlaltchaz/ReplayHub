...
import { ActivityType, Client, EmbedBuilder, GatewayIntentBits, Guild } from 'discord.js';
...
export class DiscordBotService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.encryptionKey =
      this.configService.get<string>('DISCORD_ENCRYPTION_KEY') || this.generateEncryptionKey();
  }
  ...
  async onModuleInit() {
    ...
    if (!this.encryptionKey) {
      this.logger.warn(
        'No DISCORD_ENCRYPTION_KEY configured. Bot features will use auto-generated key.'
      );
    }
      ...
      for (const config of configs) {
        if (config.botToken) {
          try {
            ...
            this.logger.error(
              `❌ Failed to connect bot for tenant ${config.tenantId}:`,
              error.message
            );
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        client.once('ready', () => {
          ...
          // Set a friendly presence so users know the bot is active
  ...
  async verifyBotToken(
    token: string
  ): Promise<{ valid: boolean; guilds?: Guild[]; botTag?: string; error?: string }> {
    try {
  ...
  async postToChannel(
    ...
    try {
      this.logger.log(
        `[postToChannel] Attempting to post to channel ${channelId} for tenant ${tenantId}`
      );
      ...
      if (!channel.isTextBased() || !('send' in channel)) {
        this.logger.warn(
          `[postToChannel] Channel ${channelId} is not a sendable text channel (type: ${channel.type})`
        );
        return false;
      }
      ...
      this.logger.error(
        `[postToChannel] ❌ Failed to post to channel ${channelId}:`,
        error.message,
        error.stack
      );
      return false;

...
export class UnifiedUserService {
    ...
    async getAccountsByEmail(email: string): Promise<UnifiedAccountsByEmail> {
        ...
        const unifiedOrgAccounts = memberships.map((membership) => this.mapMembershipToOrgAccount(membership));
    ...
    async getOrgAccountByUserIdAndTenantId(userId: string, tenantId: string): Promise<UnifiedOrgAccount | null> {
        const membership = await this.prisma.userOrganisationMembership.findUnique({
            where: {
                userId_tenantId: {
                    userId,

...
async function loginAsAdmin() {
  const response = await axios.post(
    `${API_BASE}/org/${ORG_SLUG}/auth/login`,
    {
...
async function testAssetOperations(authHeaders, testImagePath) {
  ...
  const uploadResponse = await axios.post(`${API_BASE}/org/${ORG_SLUG}/assets/upload`, formData, {
    headers: uploadHeaders,
  });

  console.log(
    `✓ Asset uploaded: ${uploadResponse.data.id} (version ${uploadResponse.data.version})`
  );
  ...
  const uploadResponse2 = await axios.post(`${API_BASE}/org/${ORG_SLUG}/assets/upload`, formData2, {
    headers: uploadHeaders2,
  });

  console.log(
    `✓ New version uploaded: ${uploadResponse2.data.id} (version ${uploadResponse2.data.version})`
  );

  // 3. List assets
  console.log('📋 Listing assets...');

  const assetsResponse = await axios.get(`${API_BASE}/org/${ORG_SLUG}/assets`, authHeaders);
...
async function testRLSAndPermissions(authHeaders, inventoryIds, assetIds) {
  ...
  const itemsResponse = await axios.get(`${API_BASE}/org/${ORG_SLUG}/inventory/items`, authHeaders);

  // Items should only show for current tenant
  const tenantItems = itemsResponse.data.data.filter(
    item => item.id === inventoryIds.item1Id || item.id === inventoryIds.item2Id
  );

...
export class PlayerService {
  ...
  async findMany(tenantId: string, query: PlayerQueryDto) {
    ...
    if (query.active !== undefined) {
      ...
      const isActive = typeof query.active === 'string' ? query.active === 'true' : query.active;
    ...
    await this.auditService.log({
      ...
      metadata: {
        before: {
          ...
          realName: (existing as any).realName ?? null,
          role: (existing as any).role ?? null,
          rank: (existing as any).rank ?? null,
          eligibility: (existing as any).eligibility ?? null,
        },
        after: {
          gamerTag: updatedPlayer.gamerTag,
          realName: (updatedPlayer as any).realName ?? null,
          role: (updatedPlayer as any).role ?? null,
          rank: (updatedPlayer as any).rank ?? null,
          eligibility: (updatedPlayer as any).eligibility ?? null,
        },
        changes: this.buildPlayerChanges(existing, updatedPlayer),
        actorEmail,
      },
    });
  ...
  async delete(
    ...
    await this.auditService.log({
      ...
      metadata: {
        gamerTag: player.gamerTag,
        realName: (player as any).realName ?? null,
  ...
  async linkToUser(
    ...
    if (
      globalUser.socialLinks &&
      (!player.socialsJson || JSON.stringify(player.socialsJson) === '{}')
    ) {
    ...
    await this.auditService.log({
      tenantId,
      action: 'player.link_user',
      entity: 'player',
      entityType: 'ORG_USER',
      entityId: id,
      orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
      description: 'Linked player to user',
      metadata: {
        playerId: id,
        gamerTag: player.gamerTag,
        globalUserId: linked.globalUserId,
  ...
  async unlinkFromUser(
    ...
    const unlinked = await this.prisma.player.update({
      where: { id },
      data: {
        globalUserId: null,
      },
    ...
    await this.auditService.log({
      tenantId,
      action: 'player.unlink_user',
      entity: 'player',
      entityType: 'ORG_USER',
      entityId: id,
      orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
      description: 'Unlinked player from user',
      metadata: {
        playerId: id,
        gamerTag: player.gamerTag,
        previousGlobalUserId: player.globalUserId,
        actorEmail,
      },
    });
  ...
  async setAvailability(
    ...
    await this.auditService.log({
      tenantId,
      action: 'player.availability.set',
      entity: 'player',
      entityType: 'ORG_USER',
      entityId: playerId,
      orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
      description: 'Updated player availability',
      metadata: {
        playerId,
        gamerTag: player.gamerTag,
  ...
  private buildPlayerChanges(before: any, after: any) {
    const fields = [
      'gamerTag',
      'realName',
      'role',
    ...
    return fields.reduce((acc: Record<string, any>, field) => {
      if (before[field] !== after[field]) {
        acc[field] = { before: before[field], after: after[field] };
      }
      return acc;
    }, {});
  }
  ...
  async updatePlayerProfile(
    ...
    await this.auditService.log({
      ...
      entityId: playerId,
      orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
      description: 'Updated own player profile',
      metadata: {
        playerId,
        gamerTagBefore: player.gamerTag,
        gamerTagAfter: updated.gamerTag,
        roleBefore: player.role,
        roleAfter: updated.role,
        rankBefore: player.rank,
        rankAfter: updated.rank,
        actorEmail,
      },
    });
  ...
  async updatePlayerSettings(
    ...
    await this.auditService.log({
      tenantId,
      action: 'player.settings.update',
      entity: 'player',
      entityType: 'ORG_USER',
      entityId: playerId,
      orgUserId: await this.resolveActorOrgUserId(tenantId, actorOrgUserId, actorEmail),
      description: 'Updated player settings',
      metadata: {
        playerId,
        gamerTag: player.gamerTag,
        statsVisibleBefore: player.statsVisible,
        statsVisibleAfter: updated.statsVisible,
        actorEmail,
      },
    });
  ...
  async syncUserDataToPlayer(tenantId: string, playerId: string, globalUserId: string) {
  ...
  async getPlayerGameStats(tenantId: string, playerId: string, requestingUserId?: string) {
    ...
    return await this.prisma.$transaction(async tx => {
      ...
      console.log(
        '[getPlayerGameStats] Player found:',
        player
          ? {
              id: player.id,
              gamerTag: player.gamerTag,
              statsVisible: player.statsVisible,
      ...
      if (player.statsVisible === false) {
        ...
        console.log('[getPlayerGameStats] Stats private check:', {
          statsVisible: player.statsVisible,
          isOwnPlayer,
        });
      ...
      try {
        ...
        console.log('[getPlayerGameStats] Stats after filtering null matches:', {
          count: stats.length,
        });
      } catch (error) {
      ...
      const recentPerformance = recentMatches.map((match: any) => ({
        ...
        avgRating:
          match.playerStats?.length > 0
            ? parseFloat(
                (
                  match.playerStats.reduce((sum: number, s: any) => sum + (s.rating || 0), 0) /

...
async function testSessionEnforcement() {
  ...
  try {
    ...
    const globalLoginResponse = await axios.post(
      `${baseUrl}/global/auth/login`,
      {
  ...
  try {
    ...
    const orgLoginResponse = await axios.post(
  ...
  if (orgCookie) {
    try {
      ...
      console.log(
        `Guards that validated: TenantGuard ✅ → OrgAuthGuard ✅ → PermissionGuard (✅/❌)`
      );
    } catch (error) {

...
async function debugRLS() {
  ...
  try {
    ...
    const userDist = await prisma.$queryRawUnsafe(
      'SELECT tenant_id, COUNT(*) as count FROM org_users GROUP BY tenant_id ORDER BY count DESC'
    );
    userDist.forEach(row => {
      const org =
        row.tenant_id === testorgId ? 'testorg' : row.tenant_id === alphaId ? 'alpha' : 'other';
    ...
    const alphaActual = await prisma.$queryRawUnsafe(
      `SELECT COUNT(*) as count FROM org_users WHERE tenant_id = '${alphaId}'`
    );
    console.log('Actual alpha users in DB:', alphaActual[0].count);

    const testorgActual = await prisma.$queryRawUnsafe(
      `SELECT COUNT(*) as count FROM org_users WHERE tenant_id = '${testorgId}'`
    );
    ...
    const currentSetting = await prisma.$queryRawUnsafe(
      "SELECT current_setting('app.tenant_id') as setting"
    );
    console.log('Current tenant_id setting:', currentSetting[0].setting);

    // Manual RLS condition test
    const manualFilter = await prisma.$queryRawUnsafe(
      "SELECT COUNT(*) as count FROM org_users WHERE tenant_id = current_setting('app.tenant_id')"
    );
    ...
    if (alphaActual[0].count === '0') {
      console.log(
        '✅ RLS working correctly - alpha org has no users, so both contexts see the same data'
      );
    } else {

...
export class PrismaTenantMiddleware {
  ...
  createMiddleware() {
    return async (params: any, next: (params: any) => Promise<any>) => {
      ...
      if (!tenantId) {
        console.warn(
          `[PrismaTenantMiddleware] No tenant context for ${model}.${action} - allowing operation`
        );
        return next(params);
      }

      console.log(
        `[PrismaTenantMiddleware] Applying tenant filter for ${model}.${action} with tenantId: ${tenantId.substring(0, 8)}...`
      );
  ...
  apply(prisma: PrismaClient): void {
    ...
    try {
      if (typeof (prisma as any).$use === 'function') {
        ...
        console.warn(
          '[PrismaTenantMiddleware] $use method not available - tenant middleware not applied'
        );
      }
    } catch (error) {

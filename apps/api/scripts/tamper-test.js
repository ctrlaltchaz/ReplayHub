...
async function tamperTest() {
    ...
    const controllerPath = path.join(__dirname, '../src/modules/inventory/controllers/inventory.controller.ts');
    ...
    try {
        ...
        console.log(`Original audit: ${violations.length === 0 ? '✅ PASS' : '❌ FAIL'} (${violations.length} violations)`);
    ...
    if (sessionCookie) {
        try {
            const response = await axios.post(`${baseUrl}/org/testorg/inventory/items`, {
            ...
            console.log(`Original POST /inventory/items: ${response.status} (${response.status === 403 ? 'properly blocked' : response.status === 401 ? 'auth required' : 'unexpected'})`);
        } catch (error) {
    ...
    try {
        ...
        console.log(`Tampered audit: ${tamperedViolations.length === 0 ? '❌ FAIL (should detect missing guards)' : '✅ PASS'} (${tamperedViolations.length} violations)`);

        // Check if guards are missing
        const hasGuardViolations = tamperedRoutes.some(route => !route.guards.includes('PermissionGuard'));
    ...
    try {
        ...
        console.log(`Restored audit: ${restoredViolations.length === 0 ? '✅ PASS' : '❌ FAIL'} (${restoredViolations.length} violations)`);

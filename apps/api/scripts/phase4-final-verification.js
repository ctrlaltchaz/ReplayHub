...
async function runFinalTests() {
  ...
  try {
    ...
    console.log(
      `✅ A1: Availability CRUD - Status: ${avail1.status}/${avail2.status}/${getAvail.status}`
    );
    ...
    console.log(
      `❌ A1: Availability CRUD failed - ${error.response?.status}: ${error.response?.data?.message}`
    );
  }
  ...
  try {
    ...
    if (error.response?.status === 409) {
      ...
      console.log(
        `❌ A2: Unexpected duplicate behavior - ${error.response?.status}: ${error.response?.data?.message}`
      );
    }
  }
  ...
  totalTests++;
  try {
    await client.post(`${API_BASE}/org/${TENANT_SLUG}/players/invalid-player-id/availability`, {
      date: '2025-10-16',
      status: 'available',
    });
  ...
  try {
    ...
    console.log(
      `❌ A4: Timezone handling failed - ${error.response?.status}: ${error.response?.data?.message}`
    );
  }
  ...
  try {
    ...
    console.log(
      `✅ B1: Lineup creation & slot management - Status: ${lineup.status}/${slots.status}`
    );
    ...
    console.log(
      `❌ B1: Lineup creation failed - ${error.response?.status}: ${error.response?.data?.message}`
    );
  }
  ...
  if (testData.lineup) {
    try {
      ...
      try {
        await client.post(`${API_BASE}/org/${TENANT_SLUG}/lineups/${testData.lineup.id}/slots`, {
          slots: [
            {
              playerId: testData.players.starterP.id,
        ...
        if (
          postPublishError.response?.status === 403 ||
          postPublishError.response?.status === 409
        ) {
          console.log(
            `✅ B2: Post-publish immutability enforced - Status: ${postPublishError.response.status}`
          );
          passCount++;
        } else {
          console.log(
            `❌ B2: Unexpected post-publish response - ${postPublishError.response?.status}`
          );
        }
      }
    } catch (error) {
      console.log(
        `❌ B2: Lineup publish failed - ${error.response?.status}: ${error.response?.data?.message}`
      );
    }
  }
  ...
  try {
    ...
    console.log(
      `❌ C2: PDF generation failed - ${error.response?.status}: ${error.response?.data?.message}`
    );
  }
  ...
  totalTests++;
  try {
    await client.get(`${API_BASE}/org/${TENANT_SLUG}/exports/roster-sheet?teamId=invalid-team-id`, {
      responseType: 'arraybuffer',
    });
  ...
  console.log('====================================');
  console.log(
    `🎯 OVERALL RESULT: ${passCount}/${totalTests} tests passed (${Math.round((passCount / totalTests) * 100)}%)`
  );
